...
const dbNotes = JSON.parse(
    fs.readFileSync(path.join(__dirname, "./dev/db/db.json"), (err, data) => {
        if (err) throw err;
    })
);
...
const dbUpdate = dbNotes => {
    fs.writeFileSync(
        path.join(__dirname, "./dev/db/db.json"),
...
app.get("./dev/public/assets/css/styles.css", (req, res) => {
    res.sendFile(path.join(__dirname, "./dev/public/assets/css/styles.css"));
});

app.get("./dev/public/index.js", (req,res) => {
    res.sendFile(path.join(__dirname, "./dev/public/index.js"));
});
